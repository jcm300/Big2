...
void fim(STATE e){
	...
	e.acao=1;
	printf("<svg height = \"120\" width = \"100\">\n");
		...
		for(v = 0; v < 13; v ++){
			...
			if(carta_existe(*mao, n, v)){
				...
				if(nroCartas(temp) == 5){
					...
					else if(comparaStraight(temp, *ultima_jogada)){
						*passar = 0;
						*mao = retira_cartas(*mao, temp);
						*ultima_jogada = temp;
						return temp;
...
int comparaSTFlush(MAO jogadaAtual, MAO jogadaAnt){
	return 1;
}
		...
		for(v = 0; v < 13; v ++){
			...
			if(carta_existe(*mao, n, v)){
				...
				if(nroCartas(temp) == 5){
					...
					else if(comparaSTFlush(temp, *ultima_jogada)){
						*passar = 0;
						*mao = retira_cartas(*mao, temp);
						*ultima_jogada = temp;
						return temp;
...
STATE joga_cartas_cpu (STATE e, int y) {
	...
	if(nro > 5){
		temp = straight(&e.mao[e.ultimo_jogador], &e.ultima_jogada, &e.passar);
		if(temp != 0)
